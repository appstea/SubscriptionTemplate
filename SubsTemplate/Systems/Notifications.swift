...
final class RegisteredNotification {
...
extension INotification {
  ...
  static func observe(on queue: OperationQueue? = nil, block: @escaping (Data) -> Void) -> RegisteredNotification {
...
extension INotification where Data == Void {

  static func post() {
    center.post(name: _name, object: nil, userInfo: ["data": Void()])
  }

  static func observe(on queue: OperationQueue? = nil, block: @escaping () -> Void) -> RegisteredNotification {
    let queue = queue ?? .current
